...
public class NodeDHT implements Runnable //extends UnicastRemoteObject implements NodeDHTInterface
{
    ...
    public void run() {
        ...
        if (this.ID == 0) {
            ...
            for (int i = 1; i <= m; i++) {
                ...
                finger[i].setStart((me.getID() + (int)Math.pow(2,i-1)) % DHTSize);
            }
            for (int i = 1; i < m; i++) {
    ...
    public static String returnAllFingers(){
        ...
        for (int i = 1; i <= m; i++) {
            response = response + finger[i].getStart() + "/" + finger[i].successor().getID() + "/"
                + finger[i].successor().getIP() + ":" + finger[i].successor().getPort() + "/";
        }
        return response;
    }
    ...
    public static void init_finger_table(Node n) throws Exception {
        ...
        String result2 = makeConnection(finger[1].successor().getIP(),finger[1].successor().getPort(),request2);
        ...
        makeConnection(finger[1].successor().getIP(),finger[1].successor().getPort(),request3);

        int normalInterval = 1;
        for (int i = 1; i <= m-1; i++) {

            myID = me.getID();
            nextID = finger[i].successor().getID();
            ...
            if ( (normalInterval==1 && (finger[i+1].getStart() >= myID && finger[i+1].getStart() <= nextID))
                ...
                finger[i+1].setSuccessor(finger[i].successor());
            } else {
                ...
                int fiSucc = finger[i+1].successor().getID();
                if (fiStart > succ)
                    succ = succ + DHTSize;
                if (fiStart > fiSucc)
    ...
    public static void update_finger_table(Node s, int i) throws Exception // RemoteException,
           {
               ...
               int normalInterval = 1;
               int myID = me.getID();
               int nextID = finger[i].successor().getID();
               if (myID >= nextID)
                   normalInterval = 0;
               else normalInterval = 1;
    ...
    public static Node find_predecessor(int id)  throws Exception
    {
        Node n = me;
        int myID = n.getID();
        int succID = finger[1].successor().getID();
        int normalInterval = 1;
        if (myID >= succID)
            normalInterval = 0;
    ...
    public static Node closet_preceding_finger(int id) //throws RemoteException
    {
        ...
        for (int i = m; i >= 1; i--) {
            int nodeID = finger[i].successor().getID();
            if (normalInterval == 1) {
